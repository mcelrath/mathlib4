...
theorem norm_nat_add_mul_I_le (k : ℕ) (t : ℝ) :
  ...
  have h3 : ‖(↑t : ℂ) * I‖ = |t| := by
    simp [norm_mul, Complex.norm_real, Complex.norm_I]
...
theorem Gamma_pure_imag_ne_zero {t : ℝ} (ht : t ≠ 0) :
  ...
  intro m; intro h
  have : (↑t * I).re = (-(↑m : ℂ)).re := congrArg Complex.re h
  simp at this
  have : (↑t * I).im = (-(↑m : ℂ)).im := congrArg Complex.im h
  simp at this
  exact ht this
...
theorem norm_Gamma_neg_mul_I (t : ℝ) :
  ...
  have h1 : (↑(-t) : ℂ) * I = starRingEnd ℂ (↑t * I) := by
    simp [Complex.conj_ofReal, map_mul, starRingEnd_self_apply, conj_I, ofReal_neg]
...
theorem norm_sq_Gamma_mul_I (t : ℝ) (ht : t ≠ 0) :
  ...
  have hnit_ne : (↑(-t) : ℂ) * I ≠ 0 := by
    ...
    have := congr_arg Complex.im h
    simp only [mul_im, ofReal_re, I_im, mul_one, ofReal_im, I_re, mul_zero,
      add_zero, zero_im, ofReal_neg, neg_mul, neg_eq_zero] at this
    exact this
