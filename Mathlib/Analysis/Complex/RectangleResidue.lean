...
* `Complex.rectWindingNumber_eq_two_pi_I`: winding number of rectangle = 2πi.
* `Complex.rectangle_residue`: rectangle boundary integral equals `2πi * g(z₀)`.
* `Complex.verticalLineShift`: shifting a vertical line integral picks up the residue.
...
private lemma vert_ftc (z₀ : ℂ) (c a b : ℝ)
  ...
  have hderiv : ∀ y ∈ [[a, b]], HasDerivAt
    ...
    have hparam : HasDerivAt (fun (t : ℝ) => ((↑c : ℂ) + ↑t * I) - z₀) I y := by
      ...
      have h' : HasDerivAt (fun t : ℝ => (↑t : ℂ)) 1 y := by
        convert h using 1 <;> simp [RCLike.ofRealCLM_apply]
      simpa [one_mul] using ((h'.mul_const I).const_add (↑c : ℂ)).sub_const z₀
...
that sum to `2πi`. -/
theorem rectWindingNumber_eq_two_pi_I (z₀ : ℂ) (c₂ c₁ T : ℝ)
    (hc : c₂ < z₀.re) (hc' : z₀.re < c₁)
    (hT : -T < z₀.im) (hT' : z₀.im < T) :
    (∫ x in c₂..c₁, ((↑x + ↑(-T) * I) - z₀)⁻¹) -
    (∫ x in c₂..c₁, ((↑x + ↑T * I) - z₀)⁻¹) +
...
private lemma integral_decompose_side (g₀ z₀ : ℂ) (F f : ℂ → ℂ) (param : ℝ → ℂ) (a b : ℝ)
  ...
  congr 1
  exact intervalIntegral.integral_const_mul g₀ _
...
theorem boundary_integral_eq_residue_times_winding
    {f g : ℂ → ℂ} {z₀ : ℂ} {c₁ c₂ T : ℝ}
    (hc : c₂ < z₀.re) (hc' : z₀.re < c₁)
    (hT : -T < z₀.im) (hT' : z₀.im < T)
...
Proved from `rectWindingNumber_eq_two_pi_I` (winding number = 2πi) and
...
theorem rectangle_residue {f g : ℂ → ℂ} {z₀ : ℂ} {c₁ c₂ T : ℝ}
  ...
  have hW := rectWindingNumber_eq_two_pi_I z₀ c₂ c₁ T hc hc' hT hT'
...
theorem verticalLineShift {f g : ℂ → ℂ} {z₀ : ℂ} {c₁ c₂ T : ℝ}
    (hc : c₂ < z₀.re) (hc' : z₀.re < c₁)
    (hT : -T < z₀.im) (hT' : z₀.im < T)
    (hg_cont : ContinuousOn g ([[c₂, c₁]] ×ℂ [[-T, T]]))
    (hg_diff : DifferentiableOn ℂ g
