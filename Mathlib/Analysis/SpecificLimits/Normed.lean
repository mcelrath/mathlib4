...
/-- `n ^ m * q ^ n` is eventually bounded by `r ^ n` for any `0 ≤ q < r`.
This is useful for showing that polynomial growth is dominated by exponential decay. -/
theorem eventually_pow_mul_pow_le_of_lt {m : ℕ} {r q : ℝ} (hq : 0 ≤ q)
    (hr : q < r) :
